...
// The method will return the phone object, if the input String parameter matches with the os attribute of the phone  object and its price attribute is greater than or equal to 50000. If any of the conditions are not met, then the 
...
public class Solution {
  ...
  public static int findPriceForGivenBrand(Phone [] phnArr , String brand){
    
    int priceSum = 0;
    for(Phone p : phnArr){
      if(p.getBrand().equalsIgnoreCase(brand)) priceSum+=p.getPrice();
    }
    return priceSum;
  }

  public static Phone getPhoneIdBasedOnOs (String os, Phone [] phnArr){
  ...
  public static void main(String[] args) {
    ...
    for(int i =0;i<4;i++){
      ...
      String brand = sc.nextLine();
       ...
       phnArr[i] = new Phone(phoneId, os, brand, price);
    }

      sc.nextLine();

      String brand = sc.nextLine();
      int priceSum = findPriceForGivenBrand(phnArr, brand);

      String os = sc.nextLine();
      Phone p = getPhoneIdBasedOnOs(os, phnArr);

      if(priceSum ==0) System.out.println("The given Brand is not available");
      else System.out.println(priceSum);

      if(p == null) System.out.println("No phones are available with specified os and price range");
      else System.out.println(p.getPhoneId());
